	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) 
		throws ServletException, IOException{
			...
			if(request.getSession().getAttribute("osoba") == null){
				request.getSession().setAttribute("osoba", new Game());
			...
			Game osoba = (Game) request.getSession().getAttribute("osoba");
			if(osoba.getDystrybutor().equalsIgnoreCase("Mezczyzna") ){
				m=1;
			}else 
			if(osoba.getDystrybutor().equalsIgnoreCase("Kobieta") ){
				k=1;
			}
			
			if(osoba.getPlatforma().toLowerCase().contains("Ksiazki".toLowerCase())) ksiazki = 1;
			if(osoba.getPlatforma().toLowerCase().contains("Plywanie".toLowerCase())) plywanie = 1;
			...
			writer.write("<html>"
					...
					+"<button type=\"button\" value=\"" + osoba.getName() +"\" onclick=\"alert('Hello world!')\">Click Me!</button>"
					+"<br> rokUr: <input type =\"int\" name=\"rokur\"value=\""+osoba.getDataWydania()+"\" /><br/>");
			...
			writer.write("<br> opis: <br> <textarea name=\"opis\" cols=\"30\" rows=\"10\">" + osoba.getCena() + "</textarea><br/>");
